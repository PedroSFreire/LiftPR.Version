...
void sampleBRDF(inout vec3 ray_dir, inout vec3 ray_pos, inout vec3 throughput, inout bool specular_bounce, inout uint seed, inout float pdf, inout float specChance, inout float diffChance) {
    pdf = 1.0f;
    float do_specular = 0.0f;
    float do_refraction = 0.0f;
    ...
    diffChance = 1.0f - (specular_chance + refraction_chance);
    pdf = max(pdf, 0.001f);
